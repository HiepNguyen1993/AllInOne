...
export class Common {
    ...
    public static tryToWait(maxLoop, time, condition: Function, callback: Function) {
        HCore.tryToFinished(maxLoop, condition, callback, time);
    }
    ...
    public static numFormat(value, forInput = true, decimalnumber = 2) {
        ...
        let intlValue;
        if (decimalnumber) {
            intlValue = Intl.NumberFormat('it-IT',
            { minimumFractionDigits: decimalnumber, maximumFractionDigits: decimalnumber }).format(value);
        }
