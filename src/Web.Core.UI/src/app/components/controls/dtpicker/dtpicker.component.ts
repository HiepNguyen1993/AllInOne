import { Component, OnInit, Input, Output, EventEmitter, AfterViewInit, ElementRef, OnChanges, forwardRef, SimpleChanges } from '@angular/core';
...
export class DtpickerComponent implements OnInit, AfterViewInit, OnChanges, ControlValueAccessor {
    ...
    bindDtPicker() {
        ...
        const isMultiDates = this.multidates !== undefined;
        ...
        this.$dtpicker.on('dp.change', (e) => {
            ...
            if (this.multidates) {
                ...
                const tempDates = Object.keys(this._dates).map(prop => {
    ...
    clearValue() {
        ...
        this.$dtpicker[0].children[0].value = '';
    ...
    loadMultiDates(dateString = null) {
        ...
        this.$dtpicker.find('td[data-day]').each((i, elem) => {
            const $elem = $(elem);
            const dataDay = $elem.attr('data-day');
    ...
    ngOnChanges(changes: SimpleChanges) {
        ...
        if (this.$dtpicker !== undefined) {
            this.$dtpicker.find('input').attr('disabled', this.isDisabled);
        }
    }
