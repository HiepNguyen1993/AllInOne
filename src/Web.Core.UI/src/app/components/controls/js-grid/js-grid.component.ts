...
import { PageResponse, PageModel } from './../../../common/models';
  ...
  appendPageSizeSelection(args) {
    ...
    args.grid._pagerContainer.find('.jsgrid-pager')
      .prepend(`<span>Limit</span>
                <select id="pageSizeCheckbox"  class="form-control" style="width: auto; display: inline-block;">
    ...
    if (this._options.downloadExcelUrl) {
      ...
      this._httpService.getQueries(this._options.downloadExcelUrl.Url, _.assign({},
        ...
        .subscribe(res => {
          ...
          const $fakeTitleRow = this.$jsGrid.find('table:first').find('tr:first').clone();
          ...
          const $fakeBodyTable = tempTable.jsGrid('option', 'data', data).find('table:last').clone();
          const $conditionRow = $fakeTitleRow.clone();
