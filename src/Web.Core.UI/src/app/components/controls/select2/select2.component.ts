...
export class Select2Component implements OnInit, AfterViewInit {
    ...
    @Input()
    public set valueTriggerChange(value: any) {
        this.requireSelect2Init();
        ...
        this.onChange(value);
        this.onTouched();
        this.onSelect2Change({target: {value: this._value}}, false);
    ...
    public get valueTriggerChange(): any {
        return this._value;
    }
    ...
    public onSelect2Change(e, isNotAllowEqualValue = true) {
        const value = e.target.value;
        // tslint:disable-next-line:triple-equals
        if (!this._originalSource || (value == this.value && isNotAllowEqualValue)) {
            return;
        }
        // tslint:disable-next-line:triple-equals
